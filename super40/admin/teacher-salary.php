...
<?php 
 	...
 	if (isset($_POST['btn_save'])) {
 		...
 		if ($run) {
			$success_message = "Salary Scale Added Successfully";
 		}
 		else {
			$error_message = "Salary Scale Add OPeration Failed";
 		}
 	}
 	...
 	if (isset($_POST['btn_sub'])) {
 		...
 		while ($row=mysqli_fetch_array($run)) {
	 		...
	 		if ($run1) { 
				...
				$success_message = "Salary has been paid to Id is : ".$row['teacher_id'];

			}
			else { 
				$error_message = "Salary has not been paid due to some errors";
	 		}
 	    }
 	}
?>
