...
<?php  
	...
	if (!$_SESSION["LoginAdmin"])
	{
		...
		$error_message_bol = false;
		$success_message_bol = false;
		$message = '';
	?>
...
// *****************************************Images upload code end here********************************************************** 
 		...
 		if ($run) {
			 ...
			 if ($run2) {
				$success_message = "Student Admitted";
				
				$success_message_bol = true;
				
			}	
			else{
				$error_message = "Student Admitted Failed";
				$error_message_bol = true;
		
			}
 		}
 		else {
			$error_message_bol = true;
			$error_message = "Semester Update Failed";
 		}
...
<?php  
	if (isset($_POST['btn_save2'])) {
		...
		if ($run3) {
			...
			$error_message_bol = true;
			$error_message = "Subject Update Failed";
		}
...
<?php  
	if (isset($_POST['btn_save3'])) {
		$semester=$_POST['semester'];
		$roll_no=$_POST['roll_no'];
		$quer3 = "UPDATE student_info SET semester = $semester WHERE roll_no = $roll_no";

		$run4 = mysqli_query($con,$quer3);
		if ($run4) {
			$success_message_bol = true;
	...
	<body>
		...
		<main role="main" class="col-xl-10 col-lg-9 col-md-8 ml-sm-auto px-md-4 w-100">
			<div class="sub-main">
				...
				<div class="col-md-12">
							<?php
								if($success_message_bol==true){
									$bg_color = "alert-success";
									$message = $success_message;
								}
								else if($error_message_bol==true){
								  		...
								  		<div class="row">
											...
											<div class="col-md-4">
												<div class="form-group">
												    ...
												    <input type="text" name="cnic" data-inputmask="'mask': '9999-9999-9999'" placeholder="XXXX-XXXX-XXXX" class="form-control">
											    </div>
											</div>
								  		</div>
								  		<div class="row">
						...
						<section class="mt-3">
							<div class="row">
								...
								<div class="col-md-12 pt-5 mb-2">
									...
									<button type="button" class="btn btn-primary ml-auto" data-toggle="modal" data-target=".bd-example-modal-lg2">Assign Semester to Student</button>
								...
								<div class="modal fade bd-example-modal-lg2" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
										<div class="modal-dialog modal-lg">
												<div class="modal-content">
													<div class="modal-header bg-info text-white">
														<h4 class="modal-title text-center">Assign Semester to Student<</h4>
													</div>
													<div class="modal-body">
														<form action="student.php" method="POST" enctype="multipart/form-data">
															<div class="row mt-3">
															...
															<div class="modal-footer">
																<input type="submit" class="btn btn-primary" name="btn_save3">
																<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
															</div>
														</form>
													</div>
							...
							<table class="w-100 table-elements mb-5 table-three-tr text-center" cellpadding="10">
								...
								<?php 
								$query="select first_name,middle_name,admission_date,last_name,current_address,session,roll_no,form_b ,profile_image,course_code from student_info ORDER BY roll_no DESC";
								$run=mysqli_query($con,$query);
								while($row=mysqli_fetch_array($run)) {?>
									<tr>
